...
class AuthLibrary
{
    ...
    public function logout()
    {
        ...
        if ($userID = $oid) $this->user = $this->commonModel->getOne($this->config->userTable,['_id' => (object)$userID]);
    ...
    public function has_perm(string $module, string $method): bool
    {
        ...
        if(empty($method)) $where=['_id'=>new ObjectId($module)];
        ...
        if (!empty($classID)) {
            ...
            foreach ($allPerms as $allPerm) {
                if ((string)$allPerm->page_id == (string)$classID->_id) $perms[] = $allPerm;
            }
            ...
            foreach ($perms as $key => $perm) {
                if ($key > 0) $c = $key - 1;
                if ($perm->create_r === true && $perms[$c]->create_r === $perm->create_r) $allPerms[0]['create_r'] = true;
                if ($perm->read_r === true && $perms[$c]->read_r === $perm->read_r) $allPerms[0]['read_r'] = true;
                if ($perm->update_r === true && $perms[$c]->update_r === $perm->update_r) $allPerms[0]['update_r'] = true;
                if ($perm->delete_r === true && $perms[$c]->delete_r === $perm->delete_r) $allPerms[0]['delete_r'] = true;
            }
