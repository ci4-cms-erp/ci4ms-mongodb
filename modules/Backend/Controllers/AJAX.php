...
class AJAX extends BaseController
{
    ...
    public function limitTags_ajax()
    {
        if($this->request->isAJAX()){
            if($this->commonModel->get_where([],'tags')===1){
                $result= $this->model->limitTags_ajax(['pivot.tagType'=>$this->request->getPost('type')],
                    [],
                    ['tag'=>true]);

                $edited=[];
                foreach ($result as $item) {
                    $edited[]=['id'=>(string)$item->_id,'value'=>$item->tag];
                }
    ...
    {
        if($this->request->isAJAX()){
            $generatedSeflink=$this->seflink($this->request->getPost('makeSeflink'));
            if($this->commonModel->get_where(['seflink'=>$generatedSeflink],'pages')===0)
                return $this->response->setJSON(['seflink'=>$generatedSeflink]);
        }
        else
            return redirect('403');
    }
