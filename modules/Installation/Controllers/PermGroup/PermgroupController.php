...
class PermgroupController extends BaseController
{
    ...
    public function group_update_post($id)
    {
        ...
        if ((bool)$result == true) {
            ...
            if ((bool)$result == false)
                ...
                return redirect()->to('/backend/groupList')->with('message', '<b>' . $this->request->getPost('groupName') . '</b> adlı grup ve yetkileri başarıyla eklendi.');
        } else
    ...
    public function user_perms($id)
    {
        ...
        $this->defData['pages'] = $this->commonModel->getList('auth_permissions_pages');
        $this->defData['group_perms'] = $this->commonModel->getOne('users', ['_id' => new ObjectId($id)]);
    ...
    public function user_perms_post($id)
    {
        ...
        $result = $this->commonModel->updateOne('users', ['_id' => new ObjectId($id)], $data);
        if ((bool)$result == false)
            return redirect()->back()->withInput()->with('error', 'Kullanıcı Yetkileri eklenemedi.');
        else
            return redirect()->to('/backend/officeWorker')->with('message', 'Kullanıcı yetkileri başarıyla eklendi.');
    }
}
